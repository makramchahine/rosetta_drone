...
UP_VEL = 0.3  # turn rate to look at new target
...
class BBCControlNode:
        ...
        while True:
            if self.logger.is_recording and self.image_msg is not None:
                ...
                saliency = convert_to_color_frame(
                    compute_visualbackprop(img=im_network, activation_model=self.conv_head))

                if not self.go_to_next:
                    # detect object/target in image
                    obj = self.best_object(saliency=saliency)
                    if obj is not None:
                        contour, centroid, area = obj
                        cv2.drawContours(saliency, contour, -1, (0, 255, 0), 3)
                ...
                resized = cv2.resize(stacked_img, dim, interpolation=cv2.INTER_NEAREST)
    ...
    def best_object(self, saliency: ndarray) -> Optional[Tuple[ndarray, float]]:
        """
        ...
        if len(average_contour_values) == 0:
            ...
            return contours[brightest], centroid, area
